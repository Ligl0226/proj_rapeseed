##==================================================================##
##--Description: GBLUP using multi-omics data for KF 13 ionomes
##--(1): WGS SNPs;
##--(2): RNAseq SNPs;
...
Sys.time()
args <- commandArgs(T)
cat("##===traitNo:",args[1],"===##\n")  ## here to index the different ionomes
...
Sys.time()
YionomeTraits <- colnames(KF_ionome)  ## 13
...
ETA4 <- list(list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))

ETA5 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),
             list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"))
ETA6 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),
             list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
ETA7 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),
             list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))
ETA8 <- list(list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"),
             list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
ETA9 <- list(list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"),
             list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))
ETA10 <- list(list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"),
              list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))
...
ETA13 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),
              list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"),
              list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))
ETA14 <- list(list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"),
              list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"),
              list(K_asv = ResKinMat$KinASVkf, model = "RKHS"))
...
Sys.time()
write.xlsx(x = resPA,file = paste("./01.resGP/KF_PA_MultiOmics_15Models_",sprintf("%02d",traitNo),Traiti,".xlsx",sep = ""),rowNames=T)

pdf(file = paste("./01.resGP/KF_PA_MultiOmics_15Models_",sprintf("%02d",traitNo),Traiti,"_boxplot.pdf",sep = ""),
    width = 8,height = 3,pointsize = 6)
  boxplot(resPA,ylim=c(min(resPA)-0.01,max(resPA)),xaxt = "n",
          main="KF_PA_GBLUP_MultiOmics_15Models",
