##==================================================================##
##--Description: GBLUP of KF203ASVs using multi-omics data
##--(1): WGS SNPs;
##--(2): RNAseq SNPs;
...
## all TRUE below.
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinWGS))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinRNAkf))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinRNAyl))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinGeneExpKF))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinGeneExpYL))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinASVkf))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$KinASVyl))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$Kin_WGS_RNAkf))
all.equal(rownames(KF_ASVs),rownames(ResKinMat$Kin_WGS_RNAyl))


##==================================================================##
##--KF: Prediction based on multi-omics data
##==================================================================##
...
Sys.time()
YionomeTraits <- colnames(KF_ASVs)  ## 203
Traiti <- YionomeTraits[traitNo]      ## for the trait traitNo
reptimes <- nrep

ETA1 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"))
ETA2 <- list(list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"))
ETA3 <- list(list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
ETA4 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"))
ETA5 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
ETA6 <- list(list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"),list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
ETA7 <- list(list(K_wgs = ResKinMat$KinWGS, model = "RKHS"),list(K_rna = ResKinMat$KinRNAkf, model = "RKHS"),list(K_geneExp = ResKinMat$KinGeneExpKF, model = "RKHS"))
...
write.xlsx(x = resPA,file = paste("./01.resGP/KF_PA_MultiOmics_7Models_203ASVs_",sprintf("%02d",traitNo),Traiti,".xlsx",sep = ""),rowNames=T)

pdf(file = paste("./01.resGP/KF_PA_MultiOmics_7Models_203ASVs_",sprintf("%02d",traitNo),Traiti,"_boxplot.pdf",sep = ""),
    width = 5,height = 3,pointsize = 6)
  boxplot(resPA,ylim=c(min(resPA)-0.01,max(resPA)),xaxt = "n",
          main="KF_PA_GBLUP_MultiOmics_7Models_203ASVs",
