...
cat("##===Path_KF_GeneExp:",args[4],"===##\n")
cat("##===Path_KF_SampName:",args[5],"===##\n")
cat("##===Path_KF_SNPsPos:",args[6],"===##\n")
cat("##===Path_KF_RNASNPs:",args[7],"===##\n")
...
Path_KF_GeneExp <- args[4]
Path_KF_SampName <- args[5]
Path_KF_SNPsPos <- args[6]
Path_KF_RNASNPs <- args[7]
Path_kinship <- args[8]
Memo <- args[9]
...
##==================================================================##
##--KF
SampleNames <- read.table(file = Path_KF_SampName)[,1]
all.equal(Rapeseed175list$KFid,SampleNames)  ## TRUE
SNPsPos <- fread(file = Path_KF_SNPsPos)
dim(SNPsPos);head(SNPsPos);tail(SNPsPos)
RNAseq_KF_SNPs <- fread(input = Path_KF_RNASNPs,drop = 1,data.table = F)
...
## all TRUE below.
all.equal(rownames(GeneExp_KF),rownames(RNAseq_KF_SNPs))
all.equal(rownames(GeneExp_KF),rownames(ResKinMat$KinWGS))
all.equal(rownames(GeneExp_KF),rownames(ResKinMat$KinRNAkf))
all.equal(rownames(GeneExp_KF),rownames(ResKinMat$KinRNAyl))
...
for (i in GeneExpScan_StartNo:GeneExpScan_EndNo) {
  ...
  y = as.matrix(GeneExp_KF[,i])
  GeneNamei <- colnames(GeneExp_KF)[i]
